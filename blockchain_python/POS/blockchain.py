...
class Block:
     def __init__(self, transactions, validator, previous_hash="0000"):
     ...
     def calculate_hash(self):
          ...
          data = str(self.timestamp) + str(self.previous_hash) + transaction_str
...
class Blockchain:
     ...
     def add_transaction(self, transaction, wallets):
          ...
          self.node.server.pending_transactions.append(transaction)
          return True

     def validate_new_block(self, validator_public_key, wallets):
          if len(self.node.server.pending_transactions) == 0:
               print("No Pending Transactions to Mine")
               return False

          transactions_to_add = []
          while len(self.node.server.pending_transactions) != 0:
               transaction = self.node.server.pending_transactions.popleft()
          ...
          #Handing Rewards
          reward_amount = 5
          validating_reward_transaction = Transaction(self.public_key, validator_public_key, reward_amount)
          validating_reward_transaction.sign_transaction(self.private_key)
          self.node.server.wallets[stringify(validator_public_key)] += reward_amount

          transactions_to_add.append(validating_reward_transaction)
          new_block = Block(transactions_to_add, validator_public_key, self.chain[-1].hash)

          if new_block.validate_block(wallets):
               self.chain.append(new_block)
               self.node.server.pending_transactions.clear()

               print("Block successfully mined & reward given!")
               return True
          else:
               self.node.server.punish_validator(self.node)
               print("Block rejected and validator punsihed")
